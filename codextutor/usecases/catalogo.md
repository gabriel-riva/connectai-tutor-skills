...
- Os casos de planilhas (orçamento, fluxo de caixa, DCF, limpeza, análise) usam a skill $spreadsheets; slides usam $slides + $imagegen; PoC visual usa ImageGen antes do build.
- "Missão de longa duração" é o /goal; "rotinas agendadas" são as Automations; "delegação no computador" é o plugin Computer Use.
- "App interno" é o plugin Sites (@sites) com D1 (dados estruturados) e R2 (arquivos); "ferramenta de linha de comando" usa $cli-creator + $skill-creator; app móvel usa o plugin Expo.
- Os showcases oficiais estão em developers.openai.com/showcase/*, com demos "Try it live" que servem para demonstração ao vivo quando o aluno quiser ver exemplos prontos.
