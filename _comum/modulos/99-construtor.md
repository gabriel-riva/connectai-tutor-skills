...
#### Criando a skill no Codex (Windows)

**Referência desta seção:** `codextutor/referencia/skills-plugins.md`, seções 2, 3 e 4.

**Passo 1: criar a pasta da skill**

No explorador de arquivos da sua pasta de oficina, crie a seguinte estrutura:
...
A pasta `.agents/skills` no diretório do projeto torna a skill disponível em todo trabalho feito nessa pasta. Para uma skill pessoal disponível em qualquer projeto, use `%USERPROFILE%\.agents\skills\`.
...
description: >
  Aplica a identidade visual da [Empresa] em relatórios, apresentações e documentos.
  Use quando precisar gerar ou formatar um artefato com as cores, logo e tom de voz oficiais.
  Não usar para tarefas sem relação com formatação ou entrega visual.
---
...
Em qualquer thread, digite `$` no compositor para ver as skills disponíveis. A skill aparece listada. Para invocar diretamente:

```
$branding-[empresa]
```

O Codex carrega o SKILL.md e segue as instruções antes de executar o que você pedir depois.
